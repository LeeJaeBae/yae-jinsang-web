...
const ADMIN_PASSWORD = "thebespoke2026!";
...
export default function Admin() {
  ...
  const extendSubscription = async (shopId: string, months: number) => {
    ...
    const base = shop.subscription_until && new Date(shop.subscription_until) > new Date()
    ...
    return (
      <main className="min-h-screen bg-[#0D0D0D] text-white flex items-center justify-center">
        <div className="w-full max-w-sm px-6">
          <h1 className="text-2xl font-black text-center mb-8">🔐 관리자 로그인</h1>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <input
              ...
              className="w-full bg-[#1A1A1A] border border-white/10 rounded-xl px-4 py-3 text-sm mb-4 focus:outline-none focus:border-[#FF3B30]/40"
            />
            <button className="w-full bg-[#FF3B30] hover:bg-[#FF3B30]/80 text-white font-bold py-3 rounded-xl transition">
              로그인
            </button>
          </form>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-white/30 text-sm mt-1">업소 관리 · 구독 관리 · 통계</p>
          </div>
          <button
        ...
        <div className="space-y-3">
          {filteredShops.map((shop) => {
            const isActive = shop.subscription_until && new Date(shop.subscription_until) > new Date();
            const daysLeft = shop.subscription_until
              ? Math.ceil((new Date(shop.subscription_until).getTime() - Date.now()) / 86400000)
              : 0;
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-white/40">
                      ...
                      <span>📅 가입 {new Date(shop.created_at).toLocaleDateString("ko")}</span>
                      {shop.subscription_until && (
                        <span>⏰ 만료 {new Date(shop.subscription_until).toLocaleDateString("ko")}</span>
                      )}
                    </div>
                  </div>
